...
def render_position_settings(tr):
    ...
    position_index_map = {pos: i for i, (_, pos) in enumerate(subtitle_positions)}
    saved_subtitle_position = config.ui.get("subtitle_position", "custom")
    saved_index = position_index_map.get(saved_subtitle_position, position_index_map["custom"])
    ...
    if subtitle_position == "custom":
        saved_custom_position = config.ui.get("custom_position", 58.0)
