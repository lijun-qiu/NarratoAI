...
# 建议片段数：按目标成片时长反推（约 10 秒/段）
TARGET_SECONDS_PER_SEGMENT = 10.0
...
def build_enhanced_mix_duration_plan(video_duration_sec: float) -> EnhancedMixDurationPlan:
    ...
    min_items = max(12, int(round(target_sec / TARGET_SECONDS_PER_SEGMENT)))
    max_items = max(min_items + 4, int(round(seconds / TARGET_SECONDS_PER_SEGMENT)))
    max_items = min(max_items, 40)

    if seconds < 120:
        narr_min, narr_max = 5, 15
        ost_min, ost_max = 3, 12
    elif seconds < 600:
        narr_min, narr_max = 8, 18
        ost_min, ost_max = 4, 12
    else:
        narr_min, narr_max = 10, 20
        ost_min, ost_max = 4, 15
