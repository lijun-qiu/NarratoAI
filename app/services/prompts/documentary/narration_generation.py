...
<output>
{
  "items": [
    ...
    {
        "_id": 2,
        "timestamp": "00:00:10,430-00:00:15,000",
        "picture": "角色激烈对峙的画面",
        "narration": "播放原片2",
        "OST": 1
    }
...
<ost_rules>
## OST 字段（必须输出）
- **OST=0**：解说片段。写入完整 narration，按 TTS 配音时长裁剪视频，不保留原声。
- **OST=1**：原声片段。保留对白/情绪高潮，narration 固定写「播放原片+序号」（如「播放原片2」），视频按 timestamp 完整裁剪。
- **禁止使用 OST=2**。

## 原声穿插策略
- 约每 2～4 个解说段插入 1 个 OST=1 原声段（对白、怒吼、关键台词、打斗音效）。
- 原声段时间戳应对准对白，单段 3～12 秒。
- 全片原声占比约 20%～35%，其余为 OST=0 解说。
</ost_rules>
...
7. 保持解说的连贯性、故事性和节奏感
8. OST=0 的 narration 建议 40～100 字，充分推进剧情；OST=1 的 narration 只写「播放原片+序号」
9. 在视觉高潮或关键对白处优先使用 OST=1，让原片说话
10. 整体风格要符合当前主流短视频平台的受欢迎特征
11. 每个 item 必须包含 OST 字段（0 或 1）
</restriction>"""
