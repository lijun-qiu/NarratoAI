...
def build_chunk_coverage_override(
    *,
    ...
    if is_fazu2_compact_settings(cfg):
        ...
        rules_block = build_fazu2_narration_copy_hard_requirements(theme)
        ...
        return (
            ...
            f"- 本块约 **{seg_target}** 个情节点（上限 {segment_max}）；全片 **{total_min}–{total_max} 段**\n"
    ...
    if is_compact_documentary_settings(cfg):
        ...
        return (
            ...
            f"{coverage_line}"
            f"- 本块 items **至少 {segment_min} 段**，目标约 **{segment_max} 段**\n"
...
def plan_narration_chunks(
    ...
    *,
    ...
    max_items_per_call = max(
        8, int(cfg.get("narration_chunk_max_items_per_call", 20) or 20)
    )
    required_chunks = 1
    if total_segment_target > 0:
        required_chunks = max(1, math.ceil(total_segment_target / max_items_per_call))

    section_counts = [1] * len(sections)
    chunk_markdowns = group_sections_into_chunks(
        sections,
        max_chars_per_chunk=max_chars,
        max_sections_per_chunk=max_sections,
    )
    if len(chunk_markdowns) < required_chunks:
        chunk_markdowns = split_sections_evenly(sections, required_chunks)

    if len(chunk_markdowns) <= 1:
        if total_segment_target > max_items_per_call and len(sections) > 1:
            chunk_markdowns = split_sections_evenly(sections, required_chunks)
        if len(chunk_markdowns) <= 1:
            return [
                NarrationChunkPlan(
...
def should_force_narration_chunking(
    ...
    *,
    ...
    if should_chunk_narration_input(narration_input, settings=cfg):
        return True
    max_items = max(8, int(cfg.get("narration_chunk_max_items_per_call", 20) or 20))
